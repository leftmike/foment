...
-- console mode api (console-port? <obj>) (set-console-input-mode! <port> editline|echo #t|#f)
-- document
...
-- 'read' signals/events from the system: these are generated by c code
-- on windows use QueueUserAPC and use alertable waits
-- on unix use pthread_kill to pass signals between threads using SIGUSR2
CrtlCNotify
-- process wide handling of ctrl-c: exit, ignore, notify

-- (handle-ctrl-c)

-- (with-notify-handler <handler> <thunk>)
-- (notify-thread <thread> <obj>): call (<handler> <obj>) in thread; exit-thread if no handler
or if handler returns
...
#define PORT_FLAG_CONSOLE_INPUT      0x00800000
...
inline int_t ConsoleInputPortP(FObject port)
{
    return(TextualPortP(port) && (AsGenericPort(port)->Flags & PORT_FLAG_CONSOLE_INPUT));
}
